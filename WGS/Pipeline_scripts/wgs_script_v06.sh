...
ls $FILEPATH/raw_seqs/ | awk 'NR % 2 == 0' | sed 's/^[^-]*-\([^_]*\).*$/\1/' > $FILEPATH/sample_list.txt
wait
...
rm $FILEPATH/pre_binned_lineages.txt

for j in ${samples[@]}
do
pr -tm -s' ' $FILEPATH/${j}/inter/${j}_lineages1.txt $FILEPATH/${j}/inter/${j}_percents1.txt $FILEPATH/${j}/binning/${j}_bin1.txt | awk -v samp=${j} -F' ' '{print $1,$2,$3,samp}' >> $FILEPATH/pre_binned_lineages.txt
done
